...
function printMat(mat, selector) {//create InnerHTML at 'selector-'container for given mat
    ...
    for (var i = 0; i < mat.length; i++) {
        ...
        for (var j = 0; j < mat[0].length; j++) {
            ...
            strHTML += `<td onclick="leftClick(this)" oncontextmenu="rightClick(this)" class="${className}" ${dataString}> <span>${display}</span></td>`
...
function getCellLocationById(cellElement) {
    ...
    let dashIdx = id.indexOf('-')
    let i = parseInt(id.substring(0, dashIdx))
    let j = parseInt(id.substring(dashIdx + 1))
    return { i: i, j: j }
}
